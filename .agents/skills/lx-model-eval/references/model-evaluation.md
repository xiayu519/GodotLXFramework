...
24 项分为 16 项路由题、6 项实施题、2 项可执行行为题。基础 foundation 为 23 项（Light），复杂 complex 为 vertical-slice、stateful-round、async-reward-restart 三项（xhigh）。full 是全部 24 项；smoke 仅两项。名称、选择范围和覆盖以 schema 为准。
...
`all_passed` 只表示所选用例全通过；`coverage.passed` 才表示指定套件完整通过；`full_coverage.passed` 才是全部 24 项通过。部分运行不会覆盖带套件名的完整报告。缺少 usage 不计为零成本成功；tool 的失败事件不是“模型自动重试次数”。

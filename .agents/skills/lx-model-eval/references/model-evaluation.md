...
Sol/high 完整套件用于工作流发布、基线重建或用户明确要求的最终验证；普通局部文档变更只跑静态检查。影响路由、授权、Skill 触发或完成语义时，在额度已确认后用一个或多个 `-CaseId` 只运行直接受影响的用例；失败修正后仍只重跑失败用例，不扩大为全套。
...
只有预检通过且已确认外部额度消耗，才运行真实 Sol/high。局部验证显式传 `-CaseId <id>`；不得把这些结果外推为当前 21 项完整通过，也不得因局部改动自动运行全套。

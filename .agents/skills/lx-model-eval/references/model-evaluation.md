...
28 项分为 20 项路由题、6 项实施题、2 项可执行行为题。基础 foundation 为 27 项（Light），复杂 complex 为 vertical-slice、stateful-round、async-reward-restart 三项（xhigh）。full 是全部 28 项；smoke 仅两项。名称、选择范围和覆盖以 schema 为准。
...
- 子代理路由新增独立任务分工/模型参数和简单任务零委派两题；普通源码、模型配置和概念讨论为负向边界。隔离 runner 禁用多代理，因此这两题只预演，不证明真实创建、提速或 token 节省；历史报告不覆盖新增题。
...
`all_passed` 只表示所选用例全通过；`coverage.passed` 才表示指定套件完整通过；`full_coverage.passed` 才是全部 28 项通过。部分运行不会覆盖带套件名的完整报告。缺少 usage 不计为零成本成功；tool 的失败事件不是“模型自动重试次数”。

...
- 当前 schema 共 21 项：两项 smoke、十五项产品/工具/迁移/控制面 outcome，以及框架内核、Skill 隔离、模型评测和 Project Knowledge 四项专用路由题。每项声明必须读取与禁止读取的 Skill。
...
只有预检通过且已确认外部额度消耗，才去掉 `-PreflightOnly` 运行 Sol/high 完整套件。任何旧 schema 结果都不能外推为当前 21 项通过。

...
- 测试由风险决定：一次聚合明确变更路径运行 `check`；相关改动、新失败或未决风险才重跑，达到根规则条件才运行 `validate`。提交/推送不单独升级范围；范围不清才用 `check --plan`，不强制每次多跑预览。
...
普通文档与指令检查不依赖引擎或模型 CLI。GitHub 验证只保留人工触发，云端环境配置不是本地提交条件。代码检查按变更域缩小静态范围，产品运行时路径映射不能省略；未知/共享工具改动保守检查，不能以局部报告替代完整证据。验证范围发生变化时补正向、负向和工具缺失回归，保持原有硬约束能拒绝错误。

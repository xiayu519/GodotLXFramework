...
无源码、只有成品/行为记录，或来源架构不可复用时，先提出目标驱动骨架；具有大量剧情、过场、任务、场景、人物、对话或战斗入口等同构编排时，默认评估统一事件脚本运行时，而不是逐步骤硬编码。`bin`、`json`、`txt`、Luban 或自定义格式应通过导入/编译归一到单一中间表示，脚本调用类型化产品模块；逐帧移动、高频战斗和 AI 仍由确定性模块负责。

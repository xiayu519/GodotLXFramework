...
只读计划的停止条件是：执行恰好一次 `migrate plan`，最终摘要回显该命令，报告引擎/模式与非空分类，说明授权边界，给出一个纵向切片和所需门禁，然后立即停止。不要读取来源文件或计划 JSON，不运行 `git status`、`inspect`、runtime、smoke、视觉比较、构建或 `validate`；这些属于实现阶段。

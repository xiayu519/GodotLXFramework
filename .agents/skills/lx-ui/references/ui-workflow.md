...
先以用户需求、已有页面和导航约定确定层级、暂停语义和 payload。只缺可逆的文案或布局细节时可作合理假设继续；若用户明确留下全屏/弹窗等未决选择，或既有事实仍不能决定公共契约，则集中询问会改变结果的问题，暂不创建未决页面。定位已有结构时读取相关场景或清单，必要时用 `inspect`；有新的证据需求才能扩大读取，不用固定工具次数代替判断。

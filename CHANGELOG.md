...
- 修复 2026-09-08 下游同步验收确认的两项上游问题：framework/export smoke 不再预先启动产品世界，失败等待清理后非零退出；连续截图清理断言报告实际/预期计数，并保存逐进程原始日志。增加配置了产品初始世界的隔离回归和 10 秒失败退出探针。
- Back 接受后绑定原激活的关闭完成任务：回调同步完成业务结果或自行关闭仍算已处理，等待清理并传播清理异常，缓存重开不受旧 Back 影响。增加 14 组自关闭、延迟清理、清理失败、重开、拒绝及无页面回归。
